...
\begin{notebox}[Destruction documentée — exécutée en toute fin]
L'environnement est \textbf{conservé} le temps de valider l'ensemble des livrables et des captures. Seule la
\textbf{prévisualisation} (\code{terraform plan -destroy}, qui ne supprime rien) a été exécutée à ce stade. La
destruction effective (\code{terraform destroy}) est la \textbf{dernière action} du projet.
\end{notebox}
...
\subsection{Commande de destruction et vérification}
\begin{codebox}{bash}
\begin{Verbatim}
terraform destroy        # confirmation interactive : yes
# Resultat attendu : Destroy complete! Resources: 21 destroyed

# Verifications apres destruction :
terraform state list                 # (vide)
az group show -n rg-shopeasy-dev      # ResourceGroupNotFound
\end{Verbatim}
\end{codebox}
\code{terraform destroy} supprime toutes les ressources gérées par le state, dans l'ordre inverse des
dépendances. La suppression du \textbf{Resource Group} entraîne celle de toutes les ressources qu'il contient
(y compris les disques managés). Les captures ayant déjà été récupérées, aucune perte n'en résulte pour le
rendu.

\begin{notebox}[Point obligatoire du TP]
Ne jamais terminer une séance sans avoir supprimé les ressources de formation. Cette étape est exécutée juste
avant la clôture du projet.
\end{notebox}

\begin{etatbox}
\begin{itemize}
  \item Prévisualisation \code{terraform plan -destroy} : \textbf{21 ressources} prêtes à être détruites (\code{0 add, 0 change}).
  \item Procédure de destruction documentée (\code{terraform destroy} + vérifications).
  \item Environnement volontairement conservé pour validation finale ; destruction = dernière action du projet.
\end{itemize}
\end{etatbox}
